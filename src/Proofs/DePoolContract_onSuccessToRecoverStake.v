...
let (round', l') := if if1 then 
                        if if2 then ({$ round with ( RoundsBase_ι_Round_ι_step , RoundsBase_ι_RoundStepP_ι_WaitingUnfreeze ) ;
                                                   ( RoundsBase_ι_Round_ι_unused , value ) $}, l) else
                        ({$ round with ( RoundsBase_ι_Round_ι_step , RoundsBase_ι_RoundStepP_ι_WaitingUnfreeze ) ;
                                       ( RoundsBase_ι_Round_ι_completionReason , RoundsBase_ι_CompletionReasonP_ι_ElectionsAreLost ) $}, l)
                           else if if3 then 
                                        let r' := {$ round with ( RoundsBase_ι_Round_ι_recoveredStake , value ) $} in
                                        if if4 then  
                                             run ( ↓ ( DePoolContract_Ф_acceptRewardAndStartRoundCompleting r'  value ) ) l 
                                               else 
                                             run ( ↓ ( DePoolContract_Ф_startRoundCompleting r' 
                                               RoundsBase_ι_CompletionReasonP_ι_ValidatorIsPunished ) ) l
