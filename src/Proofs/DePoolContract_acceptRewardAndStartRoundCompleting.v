...
let associationReward := reward * ( eval_state ( ↑12 ε DePoolContract_ι_m_associationRewardFraction ) l1 ) / 100  in
let if2 : bool := ( negb ( associationReward =? 0 ) ) in
let l2 := if if2 then exec_state (  tvm_transfer (  eval_state ( ↑12 ε DePoolContract_ι_m_association ) l1 )
                                                associationReward  
                                                false  
                                                1
