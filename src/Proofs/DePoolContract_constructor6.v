...
Lemma constructor6_eval: forall  (minStake validatorAssurance: Z)
                                 (proxyCode: TvmCell) 
                                 (validatorWallet association: Z)
                                 (participantRewardFraction validatorRewardFraction: Z)
                                 (minimumBalance : Z) (l: Ledger),
eval_state (DePoolContract_Ф_Constructor6 minStake validatorAssurance 
                                          proxyCode validatorWallet 
                                          association  participantRewardFraction 
                                          validatorRewardFraction minimumBalance) l = 
(* require(address(this).wid == 0, Errors.NOT_WORKCHAIN0); *)
...
let PROXY_CODE_HASH := eval_state (↑ε12 DePoolContract_ι_PROXY_CODE_HASH) l in
    ...
    (* require(association.isStdAddrWithoutAnyCast(), Errors.ASSOCIATION_IS_NOT_STD); *)
    let associationStdAddrWithoutAnyCast := isStdAddrWithoutAnyCast association in 
    (* require(participantRewardFraction > 0, Errors.BAD_PART_REWARD); *)
    let participantRewardFraction0 := participantRewardFraction >? 0 in
    (* require(validatorRewardFraction > 0, Errors.BAD_VALID_REWARD); *)
    let validatorRewardFraction0 := validatorRewardFraction >? 0 in
    (* require(uint(participantRewardFraction) + validatorRewardFraction <= 100, Errors.PERCENT_NOT_EQ_100); *)
    let fraction100 := participantRewardFraction + validatorRewardFraction <=? 100 in
    (* uint8 associationRewardFraction = 100 - participantRewardFraction - validatorRewardFraction; *)
let associationRewardFraction := 100 - participantRewardFraction - validatorRewardFraction in
let isFractionZero := associationRewardFraction =? 0 in 
let isAddressZero := association =? 0 in 
    (* require(isFractionZero == isAddressZero, Errors.BAD_ASSOCIATION_AND_PERCENT); // both are set or both are equal to zero *)
    let assFractionZero := eqb isFractionZero isAddressZero in
let CRITICAL_MIN_BALANCE := eval_state (↑ε12 DePoolContract_ι_CRITICAL_MIN_BALANCE) l in
    (* require(minimumBalance >= 2 * CRITICAL_MIN_BALANCE, Errors.BAD_MINIMUM_BALANCE); *)
    let minimumBalance2 := minimumBalance >=? 2 * CRITICAL_MIN_BALANCE in 
    (* require(address(this).balance >= minimumBalance + 2 * (DePoolLib.MIN_PROXY_BALANCE + DePoolLib.PROXY_CONSTRUCTOR_FEE), Errors.BAD_MINIMUM_BALANCE); *)
...
let PROXY_CONSTRUCTOR_FEE := eval_state (↑ε9 DePoolLib_ι_PROXY_CONSTRUCTOR_FEE) l in 
    let balance2 := balance >=? minimumBalance + 2*(MIN_PROXY_BALANCE + PROXY_CONSTRUCTOR_FEE) in
                ...
                if (minStakeValidator) then  
                    if (proxyCodeSame) then 
                        if (validatorStdAddrWithoutAnyCast) then 
                            ...
                            else Error (eval_state (↑ε7 Errors_ι_ASSOCIATION_IS_NOT_STD) l) 
