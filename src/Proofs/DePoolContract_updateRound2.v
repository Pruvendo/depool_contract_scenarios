...
let (round2, l'') := if if5 then 
                            if if6 then 
                                   let r := {$ round2 with (RoundsBase_ι_Round_ι_step, RoundsBase_ι_RoundStepP_ι_Completed) $} in
                                   if if7 then (r , 
                                                {$  l' With (VMState_ι_events,  (RoundCompleted (eval_state (RoundsBase_Ф_toTruncatedRound r) l')) :: 
                                                                                 oldEvents) $} )
                                          else run ( ↓ DePoolContract_Ф_startRoundCompleting round2 round2 ->> RoundsBase_ι_Round_ι_completionReason) l'
...
replace RoundsBase_ι_Round_ι_step with RoundsBase_ι_RoundStepP_ι_Completing . idtac.
auto.
destruct RoundsBase_ι_Round_ι_step; auto; try discriminate.
...
let round2 := if if5 then 
                            if if6 then 
                                   let r := {$ round2 with (RoundsBase_ι_Round_ι_step, RoundsBase_ι_RoundStepP_ι_Completed) $} in
                                   if if7 then r
                                          else eval_state ( ↓ DePoolContract_Ф_startRoundCompleting round2 round2 ->> RoundsBase_ι_Round_ι_completionReason) l'
                                    else if if8 then {$ round2 with (RoundsBase_ι_Round_ι_step, RoundsBase_ι_RoundStepP_ι_WaitingReward) $} 
                                                else round2
                     else round2 in 
                     round2.
