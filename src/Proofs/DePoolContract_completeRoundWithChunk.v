...
(* round = _returnOrReinvest(round, chunkSize); *)
let round' := eval_state ( ↓ DePoolContract_Ф__returnOrReinvest round Л_chunkSize ) l  in 
...
let if2 : bool := negb ( eqb ( round ->> RoundsBase_ι_Round_ι_step ) RoundsBase_ι_RoundStepP_ι_Completing ) in
(* round = _returnOrReinvest(round, chunkSize); *)
let (round', l__returnOrReinvest) := run ( ↓ DePoolContract_Ф__returnOrReinvest round Л_chunkSize ) l  in 
...
if req then 
   if if1 then l
                ...
                if if2 then l else match round' with
