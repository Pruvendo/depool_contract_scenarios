...
(* Notation "'math->foo' '(!' a , b '!)'" := (do a' ← a; do b' ← b; return! (xIntPlus a' b')) (at level 30, right associativity).
Compute exec_state (↑17 U1! LocalState_ι_withdrawStakeInPoolingRound_Л_targetAmount := math->foo (! (ε LocalState_ι_withdrawStakeInPoolingRound_Л_targetAmount) ,
(D2! LocalState_ι_withdrawStakeInPoolingRound_Л_sv ^^ RoundsBase_ι_StakeValue_ι_ordinary) !) )  {$ default With LocalState_ι_withdrawStakeInPoolingRound_Л_sv := 
                             {$ default with RoundsBase_ι_StakeValue_ι_ordinary := 10 $} $} . *)
...
Lemma RoundsBase_Ф_withdrawStakeInPoolingRound'_exec : forall (participant : DePoolLib_ι_Participant ) 
    ...
    let stakeSum_run := run (↓ RoundsBase_Ф_stakeSum newsv) l in 
    let stakeSum := fst stakeSum_run in
    ...
    if (xMaybeIsSome optsv: bool) then {$ l With RoundsBase_ι_m_rounds := oldRounds [roundQty - 2] ← newRound0 $} 
                            else l.
Proof. 
  intros.
  destruct l.  
  ...
  remember oldRound0. 
  compute in Heqr. idtac.
  rewrite <- ?Heqr. idtac.
  remember optsv as o.
  compute in Heqo. idtac.
  rewrite <- ?Heqo. idtac.
 

  repeat rewrite ifSimpleState. repeat rewrite ifFunApply.

  (* no such participant in round *)
  case_eq o; intros; auto. idtac.

  (* restLessThanMin = sv.ordinary <? minStake *)
  destruct r0.
  repeat rewrite ifSimpleState. repeat rewrite ifFunApply.
 
  remember restLessThanMin.
  compute in Heqb.
  rewrite H in Heqb.
  rewrite <- ?Heqb.
  case_eq b; intros; auto. idtac.
  ...
  apply ledgerEq; auto.
  compute. idtac. 
  apply RoundsBaseEq; auto.
  compute.
  enough (forall X k k' x x' (m m': XHMap Z X), k = k' -> x = x' -> m =  m' -> hmapInsert Z.eqb k x m  = hmapInsert Z.eqb k' x' m').
  destruct r.
  destruct Ledger_ι_RoundsBase.
  apply H1 ; auto.
  apply RoundEq; auto.
  compute. lia.
  intros. rewrite H1. rewrite H2.  rewrite H3. auto.
...
Lemma RoundsBase_Ф_withdrawStakeInPoolingRound'_eval : forall (participant : DePoolLib_ι_Participant ) 
    ...
    if (xMaybeIsSome optsv: bool) then xValue ( targetAmount'' , newParticipant )
                            else xError (0, participant).                                             
Proof. 
  intros.
  destruct l.  
  compute. idtac.
...
auto.
replace (0 + 0 + 0) with 0 in H1 by lia.
rewrite Z.eqb_refl in  H1.
discriminate.
...
Lemma RoundsBase_Ф_withdrawStakeInPoolingRound_eval : forall (participant : DePoolLib_ι_Participant ) 
    ...
    let newParticipant := {$ participant with DePoolLib_ι_Participant_ι_roundQty := newParticipantRoundQty $} in 
    eval_state (↓ RoundsBase_Ф_withdrawStakeInPoolingRound participant participantAddress targetAmount minStake) l = 
    if (xMaybeIsSome optsv: bool) then  ( targetAmount'' , newParticipant )
                            else  (0, participant).
 Proof.
  intros.
